...
$ftp_directory_terra = '/MSPP/data/level2/mod14/';
...
//$fp = fopen($directory."files_list_terra.csv", "x"); // Open a files list csv file if it doesn't exist already. 
$opencsv=fopen($directory."files_list_terra.csv", "a+");
$file_list_rows = fgetcsv($opencsv, 1024, ' '); ///get the file content without empty lines.  
//print_r($file_list_rows); 
/**
*Check if the latest ftp file is in files_list_terra.csv. If it doesn't exist, copy the latest file from remote. 
...
*@param $file_list_rows (array) list of files in the files_list_terra.csv
...
function find_latest_txt($directory_url,  $file_list_rows, $latest_ftp_file, $ftp_directory_url) {
	...
	if ($file_list_rows===false) {
		...
		$check_latest_ftp_result = array_search($latest_ftp_file, $file_list_rows, true); //Check if the latest ftp file is in files_list_terra.csv
		if ( $check_latest_ftp_result===false ) { // If the latest file on the processing server are not in the file_list.csv
				...
				$files_list .= file_get_contents($directory_url."files_list_terra.csv"); //Keep existing file list. 
				file_put_contents($directory_url."files_list_terra.csv", $files_list); //Add the latest file name in the list. 		
...
$found_file_name = find_latest_txt($directory, $file_list_rows, $most_recent_ftp_file, $ftp_directory_terra); // $found_file_name could be picked by openlayers to load json into map 
/**
...
function upload_file_to_db($data_directory, $found_file_name) {
	...
	if (($handle = fopen($data_directory.$csv_file_name, 'r')) === false) { //If opening fails, show opening error. 
		...
		while($row = fgetcsv($handle, 1024, ',')) { // Get csv rows
			...
			VALUES ('{$upload_datetime}', '{$date}', '{$date}', '{$datetime}', '{$time}', '{$latitude}', '{$longitude}', '{$kelvin}', '{$scan_size}', '{$track_size}', '{$confidence}', '{$satellite}', ST_SetSRID(ST_MakePoint({$longitude}, {$latitude}), 900913), 'Terra');";
		...
		//rename($data_directory.$found_file_name, $data_directory."backup/terra/".$found_file_name);
...
if ($found_file_name=="failed"){
	...
	send_message_alert(); 
	array_map('unlink', glob( $directory."MOD14*.csv")); 
